...
def softmax(src: Tensor, index, num_nodes: int):
    # Pure PyTorch segment-wise softmax over target node index.
    # src: [E, H], index: [E], output: [E, H]
    N = int(num_nodes)
    if src.numel() == 0:
        return src
    index = index.long()
    expand_index = index.view(-1, *([1] * (src.dim() - 1))).expand_as(src)
    max_buf = src.new_full((N, *src.shape[1:]), -torch.inf)
    max_buf.scatter_reduce_(0, expand_index, src, reduce='amax', include_self=True)
    max_per_edge = max_buf[index]
    out = torch.exp(src - max_per_edge)
    sum_buf = src.new_zeros((N, *src.shape[1:]))
    sum_buf.scatter_add_(0, expand_index, out)
    return out / (sum_buf[index] + 1e-9)
...
class Intra_Inter_Patch_Graph_Layer(nn.Module):
    """
    ...
    def __init__(self, n_heads=2, d_input=6, d_k=6, alpha=0.9, patch_layer=1, res=1, **kwargs):
        super(Intra_Inter_Patch_Graph_Layer, self).__init__()
    ...
    def forward(self, x, edge_index, edge_value, time_nodes, edge_same_time_diff_var, edge_diff_time_same_var, edge_diff_time_diff_var, n_layer):
        ...
        message = self.message(x[source_nodes], x[target_nodes], target_nodes, edge_value,
                               ...
                               n_layer=n_layer, size_i=x.size(0))
        aggr_out = torch.zeros_like(x)
        aggr_out.index_add_(0, target_nodes, message)
        return self.update(aggr_out, residual)
