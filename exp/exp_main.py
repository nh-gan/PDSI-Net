...
class Exp_Main(Exp_Basic):
    ...
    def vali(
        ...
        if len(total_loss) == 0:
            total_loss = np.inf
        elif self.configs.loss == "MSE" and np.sum(total_weight) > 0:
            # Each batch MSE is already divided by its number of observed
            # targets.  Weighting it by that count reconstructs the global
            # observation-level MSE used by the final test metric.  A plain
            # average makes sparse and dense batches count equally and causes
            # checkpoint selection to depend on validation batch grouping.
            total_loss = np.average(total_loss, weights=total_weight)
        else:
            ...
            for epoch in tqdm(range(self.configs.train_epochs), desc="Epochs"):
                ...
                with tqdm(total=len(train_loader), leave=False, desc="Training") as it:
                    ...
                    for i, batch in enumerate(train_loader):
                        ...
                        if (
                            self.configs.rastgi_debug
                            and epoch % 10 == 0
                            and i == 0
                            and accelerator.is_main_process
                        ):
                            core_model = accelerator.unwrap_model(model_train)
                            lambda_grad = core_model.lambda_dc.grad
                            logger.info(
                                "RASTGI-HG debug epoch=%d hyper_active=%s "
                                "lambda_dc=%.6f lambda_grad=%s gate_mean=%.6f "
                                "enhance_density=%.6f",
                                epoch,
                                bool(core_model.last_hyper_active.item()),
                                core_model.lambda_dc.detach().item(),
                                (
                                    "None"
                                    if lambda_grad is None
                                    else f"{lambda_grad.detach().item():.3e}"
                                ),
                                core_model.last_gate_mean.item(),
                                core_model.graph_builder.last_enhance_density.item(),
                            )
                            gate_mean = core_model.last_gate_mean.item()
                            enhance_density = (
                                core_model.graph_builder.last_enhance_density.item()
                            )
                            if lambda_grad is None or abs(lambda_grad.item()) < 1e-12:
                                logger.warning("RASTGI-HG lambda_dc gradient vanished")
                            if core_model.last_hyper_active.item() and (
                                gate_mean < 0.01 or gate_mean > 0.99
                            ):
                                logger.warning(
                                    "RASTGI-HG fusion gate is saturated: %.6f",
                                    gate_mean,
                                )
                            if enhance_density > 0.8:
                                logger.warning(
                                    "RASTGI-HG enhancement edges are too dense: %.6f",
                                    enhance_density,
                                )
                        model_optim.step()
                ...
                if epoch % self.configs.val_interval == 0:
                    ...
                    if (
                        early_stopping.val_loss_min == vali_loss
                        and accelerator.is_main_process
                    ):
                        best_epoch = epoch + 1
                        logger.info(
                            "Best Epoch: %d | Validation MSE: %.8f",
                            best_epoch,
                            float(vali_loss),
                        )
                        with open(path / "best_validation.json", "w") as f:
                            json.dump(
                                {
                                    "Best Epoch": best_epoch,
                                    "Validation MSE": float(vali_loss),
                                },
                                f,
                                indent=2,
                            )
    ...
    def test(self) -> None:
        ...
        if self.configs.test_flop:
            ...
            # 2. 从加载器中取出数据字典
            # DataLoader 直接返回一个字典，我们用一个变量接住它
            batch_dict = next(iter(test_loader))

            # (可选) 如果你想查看字典内容，可以使用下面的调试代码
            # print("DataLoader returned a dictionary with these keys:", batch_dict.keys())
            ...
            # 3. 将字典中所有张量的值移动到 GPU 设备
            # 这是一个非常简洁和高效的做法
            real_inputs = {
                key: value.to(self.device) for key, value in batch_dict.items() if hasattr(value, 'to')
            }

            logger.info("Using a real data batch (dictionary format) for FLOPs test.")

            test_params_and_flops(
                model=model,
                dummy_inputs=real_inputs,  # <--- 直接使用这个完美的字典
                model_id=self.configs.model_id,
                task_key=task_key
            )
            exit(0)
        ...
        if self.configs.checkpoints_test is None:
            ...
            if self.configs.load_checkpoints_test:
                try:
                    ...
                    child_folders = []
                    for entry in checkpoint_location.iterdir():
                        if not entry.is_dir():
                            continue
                        try:
                            timestamp = datetime.datetime.strptime(
                                entry.name, "%Y_%m%d_%H%M"
                            )
                        except ValueError:
                            continue
                        child_folders.append((timestamp, entry))
                    if len(child_folders) == 0:
                        ...
                        exit(1)
                    latest_folder: str = max(
                        child_folders, key=lambda item: item[0]
                    )[1].name
        ...
        for itr_i in range(actual_itrs):
            ...
            if metrics is not None:
                ...
                # Unified offline evaluation artifact for fair paper comparison.
                # All models save the same pred/true/mask arrays.
                if array_dict.get("pred") is not None and array_dict.get("y") is not None:
                    unified_mask = array_dict.get("y_mask")
                    if unified_mask is None:
                        unified_mask = np.ones_like(array_dict["y"], dtype=bool)
                    np.savez_compressed(
                        folder_path / "unified_eval.npz",
                        pred=array_dict["pred"],
                        true=array_dict["y"],
                        mask=unified_mask,
                        sample_ID=array_dict.get("sample_ID"),
                    )
                    with open(folder_path / "metric_protocol.json", "w") as f_protocol:
                        json.dump({
                            "protocol": "per-variable masked MAE/MSE/RMSE",
                            "array_file": "unified_eval.npz",
                            "shape": "[B, pred_len, N]",
                            "mask": "1/True means observed target position",
                            "space": "normalized model output space unless the dataset pipeline inverse-transforms before output",
                        }, f_protocol, indent=2)

                if self.configs.model_name == "ASTGI":
                    core_model = accelerator.unwrap_model(model_test)
                    if hasattr(core_model, "gaussian_artifacts"):
                        artifacts = {
                            name: tensor.detach().float().cpu()
                            for name, tensor in core_model.gaussian_artifacts().items()
                        }
                        torch.save(
                            artifacts,
                            folder_path / "gaussian_artifacts.pt",
                        )
                        np.save(
                            folder_path / "Gaussian_mu.npy",
                            artifacts["Gaussian_mu"].numpy(),
                        )
                        np.save(
                            folder_path / "Gaussian_cov.npy",
                            artifacts["Gaussian_cov"].numpy(),
                        )
                        np.save(
                            folder_path / "Gaussian_adjacency.npy",
                            artifacts["Gaussian_adjacency"].numpy(),
                        )
                        metrics_with_rmse = dict(metrics)
                        metrics_with_rmse["RMSE"] = float(
                            np.sqrt(metrics["MSE"])
                        )
                        best_validation_file = (
                            checkpoint_location_itr / "best_validation.json"
                        )
                        if best_validation_file.exists():
                            with open(best_validation_file, "r") as f:
                                best_validation = json.load(f)
                            metrics_with_rmse.update(best_validation)
                        with open(folder_path / "astgi_gs_summary.json", "w") as f:
                            json.dump(metrics_with_rmse, f, indent=2)
                        logger.info(
                            "ASTGI-GS Results: Best Epoch=%s | "
                            "Validation MSE=%s | Test MSE=%.8f | "
                            "Test MAE=%.8f | RMSE=%.8f",
                            metrics_with_rmse.get("Best Epoch", "N/A"),
                            metrics_with_rmse.get("Validation MSE", "N/A"),
                            float(metrics["MSE"]),
                            float(metrics["MAE"]),
                            metrics_with_rmse["RMSE"],
                        )
